...
STOCK_CODE = "1231"  # 台股股票代號
TARGET_KEYS = ["營業收入合計", "營業成本合計", "營業利益（損失）", "所得稅費用（利益）", "所得稅費用（利益）合計", "本期稅前淨利（淨損）", 
               "現金及約當現金", 
               "短期借款", "應付短期票券", "長期借款", "租賃負債－流動", "租賃負債－非流動", "應付公司債", 
               "權益總額", 
               "財務成本淨額", "支付之利息", 
               "所得稅費用（利益）合計", "繼續營業單位稅前淨利（淨損）", "退還（支付）之所得稅"]  # 目標會計名稱(IFRSs 前無對應代號)
...
# 抓取 IFRSs 前的財報資料
def fetch_html_with_IFRSs_before(year, stock_code, season, table_index):
    '''
    table_index:
    資產負債表:1
    綜合損益表:2
    現金流量表:3
    '''
    url = f"https://mopsov.twse.com.tw/server-java/t164sb01?t203sb01Form=t203sb01Form&step=1&CO_ID={stock_code}&SYEAR={year}&SSEASON={season}&REPORT_ID=C"
    res = requests.get(url)
    if res.status_code != 200:
        print(f"網站資料請求失敗或該資料尚不存在，狀態碼：{res.status_code}")
        return None
    ...
    res.encoding = "big5-hkscs"
    tree = html.fromstring(res.text)
    table = tree.xpath(f'//*[@id="content_d"]/center/table[{table_index}]')[0]
    ...
    for tr in table.xpath('.//tr'):
        ...
        if cells and len(cells) > 1:
            # 第一欄保留中文
            name = extract_chinese(cells[0])
            
            # 第二欄去掉逗號轉整數
            value_str = cells[1].replace(',', '').replace('(', '-').replace(')', '')
            try:
                value = int(value_str)
            except ValueError:
                value = None
...
if __name__ == "__main__":
    ...
    for year in range(98 + 1911, 101 + 1911):
        for season in [1, 2, 3, 4]:
            for type in [1, 2, 3]:
                data = fetch_html_with_IFRSs_before(year, 
                                                    STOCK_CODE, 
                                                    season, 
                                                    type)
